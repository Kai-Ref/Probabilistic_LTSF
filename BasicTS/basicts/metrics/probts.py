...
from .prob_metrics import crps, nll_loss
...
class Evaluator:
    def __init__(self, metrics='all', quantiles=[], distribution_type=None, num_samples=100):
        """
        ...
        if len(quantiles) == 0: # compute the QL across the 10 equally spaced quantiles
            quantiles_num = 20
        ...
        self.__all_metrics__ = ["MSE", "abs_error", "abs_target_sum", "abs_target_mean",
                                ...
                                "mean_absolute_QuantileLoss", "CRPS", "MAE_Coverage", "NLL", "VS", "ES"]
        if metrics == "all":
            self.metrics = self.__all_metrics__
        else:
            # check that the provided metrics are valid
            assert all([m in self.__all_metrics__ for m in metrics]) 
            self.metrics = metrics
        
        self.distribution = True if self.distribution_type not in ['quantile', 'i_quantile', None] else False
    ...
    def get_metrics(self, targets, forecasts, seasonal_error=None, samples_dim=1, loss_weights=None):
        ...
        for q in self.quantiles:
            ...
            metrics[f"QuantileLoss[{q}]"] = quant_loss
            metrics[f"wQuantileLoss[{q}]"] = quant_loss / metrics["abs_target_sum"]
            metrics[f"Coverage[{q}]"] = coverage(targets, q_forecasts)

        metrics["mean_absolute_QuantileLoss"] = torch.mean(
            torch.stack([metrics[f"QuantileLoss[{q}]"] for q in self.quantiles]), dim=0
        )
        metrics["CRPS_2"] = torch.mean(
            torch.stack([metrics[f"wQuantileLoss[{q}]"] for q in self.quantiles]), dim=0
        )
    ...
    def __call__(self, prediction: torch.Tensor, target: torch.Tensor, null_val: float = np.nan):
        ...
        if self.distribution:
            # 1. Compute exact metrics, based off distributional forecasts
            if 'NLL' in self.metrics:
                output_metrics['NLL'] = nll_loss(prediction, target, self.distribution_type, null_val=null_val)
            if 'CRPS' in self.metrics:
                # TODO handle weibull and dirichlet
                output_metrics['CRPS'] = crps(prediction, target, self.distribution_type, null_val=null_val)

            # 2. Sample to evaluate approximate metrics
            samples = self.prob_head.sample(prediction, num_samples=self.num_samples) # [samples x bs x seq_len x nvars]
            samples = samples.permute(1, 2, 3, 0).detach().cpu()      # [bs x seq_len x nvars x samples]

            # 3. Compute approximate metrics
            if 'CRPS' in self.metrics:
                output_metrics['CRPS_Sample'] = np.mean(sr.crps_ensemble(target, samples, estimator='pwm'))
                output_metrics['CRPS_Sum_Sample'] = np.mean(sr.crps_ensemble(target.sum(axis=-1), samples.sum(axis=-2), estimator='pwm'))
            if 'VS' in self.metrics:# permute the target and samples so that the last axis corresponds to the variables i.e. seq_len
                output_metrics['VS_0.5_Sample'] = np.mean(sr.variogram_score(target.permute(0, 2, 1), samples.permute(0, 2, 3, 1), p=0.5))
                output_metrics['VS_1_Sample']= np.mean(sr.variogram_score(target.permute(0, 2, 1), samples.permute(0, 2, 3, 1), p=1.0))
                output_metrics['VS_2_Sample'] = np.mean(sr.variogram_score(target.permute(0, 2, 1), samples.permute(0, 2, 3, 1), p=2.0))
            if 'ES' in self.metrics:
                output_metrics['ES_Sample'] = np.mean(sr.energy_score(target.permute(0, 2, 1), samples.permute(0, 2, 3, 1)))
            
            metrics = self.get_metrics(target, samples, seasonal_error=None, samples_dim=-1, loss_weights=None)
            metrics_sum = self.get_metrics(target.sum(axis=-1), samples.sum(axis=-2), samples_dim=-1)
        ...
        else:#TODO quantile, iquantile and point forecasts
