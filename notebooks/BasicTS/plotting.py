...
def get_predictions(runner, data_loader="test", num_samples=0):
    if data_loader == "test":
    ...
    with torch.no_grad():
        for batch_idx, batch in tqdm(enumerate(dataloader)):
            ...
            if num_samples >0:
                forecasts = model.sample(model_return['prediction'], num_samples=num_samples)
            else:
...
class Plotting:
    def __init__(self, runner):
        ...
        self.distribution_type = runner.distribution_type
        self.density_available = True if self.distribution_type not in ["quantile", "i_quantile", "", None] else False
        if not self.density_available:
            assert runner.quantiles is not None


    def plot_samples(self, num_samples=1):
        pass
        
    def plot_time_series(self, past_actuals, future_actuals, predictions, windows, series, ci=[5, 95], quantile_levels=[]):
    ...
    def plot_qq_coverage(self, future_actuals, predictions, windows, series, quantile_levels=[0.05, 0.5, 0.95], lower=False, quantile_regression=False):
        """
